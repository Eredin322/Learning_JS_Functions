...
document.querySelector('.b-1').addEventListener('click', function () {
    document.querySelector('.out-1').innerHTML = t1(120, 140);
})
...
function t3(min = 0, max = 100) {
    const rand = Math.floor(min - 0.5 + Math.random() * (max - min + 1));
    document.querySelector('.out-3').innerHTML = rand;
}
...
function t4(a, b, c) {
    if (b === 0) {
        ...
        document.querySelector('.out-4').innerHTML = Number(a / b).toFixed(2);
    }
}
...
function t5(a, b, c = 0) {
    if (b === 0) {
        ...
        document.querySelector('.out-5').innerHTML = Number(a / b).toFixed(2);
    }
}
...
function t11() {
    out11.innerHTML = Array.from(arguments).reduce((accum, item) => accum + item);
}
...
function t12(...args) {
    out12.innerHTML = args.reduce((accum, item) => accum + item);
}
